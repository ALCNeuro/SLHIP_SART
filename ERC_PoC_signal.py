...
channels_oi =  ['F3', 'TP9', 'TP10', 'VEOG', 'ECG', 'RESP']
...
for i, file_path in enumerate(files):
    ...
    raw.filter(1,10, picks=['VEOG', 'RESP'])
    raw.filter(1,20, picks=['ECG'])
    sf = raw.info['sfreq']
    
    channels_names= ['F3', 'VEOG', 'ECG', 'RESP']
    ...
    for idx, event in enumerate(ms_probes[:, 0]):
        ...
        fig.text(
            0.98, 
            0.01, 
            f"Code: {this_subid_code}_session_{session}_probe_{idx}", 
            ha="right", 
            va="bottom",
            fontsize=8, 
            color="red", 
        ...
        this_savepath = os.path.join(
            path_scoring_coded, 
            f"{this_subid_code}_session_{session}_probe_{idx}.png"
            )
