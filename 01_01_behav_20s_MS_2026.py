...
sub_df = df.loc[(df.mindstate != 'MISS') & (df.mindstate != "MW_E")]
sub_df = sub_df.loc[sub_df.sub_id != "sub_HI_006"] # HI temps court !
...
sub_df.to_csv(os.path.join(behavpath, "C1_IH_N1", "df_20s_behav.csv"))
...
# Radar plot settings
colors = {'HS': '#8d99ae', 'N1': '#d00000', 'HI' : '#ffc300'}
...
fig.tight_layout()
plt.savefig(
    os.path.join(behavpath, "C1_IH_N1", "radar_plot_mindstates.png"), 
    dpi=200
    )
plt.show()
...
for sub_id in sub_df.sub_id.unique() :    
    ...
    for dt in this_df.daytime.unique() :
        ...
        for mindstate in ['ON', 'MW_I', 'MW_H', 'MB', 'FORGOT'] :
...
plt.savefig(os.path.join(behavpath, "C1_IH_N1", "point_strip_per_mindstates_by_subtype.png"), dpi=200)

# %% Stats Mindstate per ST 

pd.options.display.float_format = '{:,.3f}'.format

temp_df = df_mindstate.copy().loc[df_mindstate.mindstate!="MW_E"]

foi = ["mindstate", "contrast", "ß", "p_val", "p_corr"]
dic = {f : [] for f in foi}

contrasts = [["HS", "N1"], ["HS", "HI"], ["HI", "N1"]]

for mindstate in ['ON', 'MW_I', 'MW_H', 'MB', 'FORGOT'] :
    for contrast in contrasts :
        
        formula = f'percentage ~ C(mindstate, Treatment("{mindstate}")) * C(subtype, Treatment("{contrast[0]}"))'
        model = smf.mixedlm(
            formula, 
            temp_df, 
        ...
        dic["mindstate"].append(mindstate)
        dic["contrast"].append(f"{contrast[1]} > {contrast[0]}")
        dic["ß"].append(
            model_result.params[f'C(subtype, Treatment("{contrast[0]}"))[T.{contrast[1]}]'])
        dic["p_val"].append(
            model_result.pvalues[f'C(subtype, Treatment("{contrast[0]}"))[T.{contrast[1]}]'])
...
df_sleepi.to_csv(os.path.join(behavpath, "sleepiness_df.csv"))
...
df_vol.to_csv(os.path.join(behavpath, "C1_IH_N1", "voluntary_df.csv"))

# %% Per Sleepi Pointplot [ST Diff]

data = df_sleepi.copy().drop(
...
fig, axes = plt.subplots(nrows = 3, ncols = 1, figsize = (12, 10), sharex = True, sharey = True)

for i, subtype in enumerate(["HS", "N1", "HI"]) :
    ax = axes[i]
    sns.barplot(
...
plt.savefig(os.path.join(
    behavpath, "C1_IH_N1", "miss_fa_rt_subtype.png"), 
    dpi=200
    )
...
ticks = {
    ...
    "miss": [np.linspace(0,40,5), np.round(np.linspace(0,40,5),0).astype(int)],
    "false_alarms": [np.linspace(0,80,5), np.round(np.linspace(0,80,5),0).astype(int)],
...
for feat in feats :
    ...
    plt.savefig(os.path.join(
        behavpath, "C1_IH_N1", f"{feat}_boxplot_groupdiff.png"
        ), dpi=300)
...
model_formula = 'voluntary ~ C(subtype, Treatment("HS"))'
model = smf.mixedlm(
    model_formula, 
    temp_df, 
    groups=temp_df['sub_id'], 
...
for feat in feats :
    ...
    plt.savefig(os.path.join(
        behavpath, "C1_IH_N1", f"{feat}_boxplot_msonly.png"
        ), dpi=300)
...
fig, axs = plt.subplots(nrows = 1, ncols = len(feats), figsize = (10, 3))
...
plt.savefig(os.path.join(
    behavpath, "C1_IH_N1", "behav_boxplot_ST_only.png"
    ), dpi=300)
...
foi = ["feature", "contrast", "ß", "p_val", "p_corr"]
dic = {f : [] for f in foi}
...
for feature in feats :
    ...
    for contrast in contrasts :
        ...
        formula = f'{feature} ~ sleepiness * C(subtype, Treatment("{contrast[0]}"))'
        model = smf.mixedlm(
            formula, 
            temp_df, 
            groups=temp_df['sub_id'], 
...
transi_hi = av_transi.loc[av_transi.subtype=="HI"]

transi_n1.drop(columns="subtype", inplace=True)
transi_hs.drop(columns="subtype", inplace=True)
transi_hi.drop(columns="subtype", inplace=True)
...
print(transi_hs)
print("\nWeighted transition matrix for IH:")
print(transi_hi)
...
plt.savefig(os.path.join(
    behavpath, "C1_IH_N1", "transmat_pruned_weighted_nt1.png"
    ), dpi=300)
...
f.tight_layout(pad=1)
plt.savefig(os.path.join(
    behavpath, "C1_IH_N1", "transmat_pruned_weighted_ctl.png"
    ), dpi=300)
...
f.tight_layout(pad=1)
plt.savefig(os.path.join(
    behavpath, "C1_IH_N1", "transmat_pruned_weighted_hi.png"
    ), dpi=300)
